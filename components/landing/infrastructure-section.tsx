...
const capabilities = [
  { name: "Hybrid RAG Search", detail: "Semantic + keyword retrieval", status: "ready" },
  { name: "Automatic Citations", detail: "Source-backed responses", status: "ready" },
  { name: "Omnichannel AI", detail: "WhatsApp • Teams • Slack", status: "ready" },
  { name: "Enterprise Security", detail: "RBAC • Audit Logs • Encryption", status: "ready" },
];
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">One</span>
                <span className="text-2xl text-muted-foreground">knowledge base</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Connect PDFs, Notion, SharePoint, Google Drive, and websites into
                a single AI knowledge source.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">Platform availability</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">&lt;340ms</span>
              <span className="block text-sm text-muted-foreground mt-2">Average AI response</span>
            </div>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className={`w-2 h-2 rounded-full transition-colors ${
                  activeCapability === index ? "bg-[#eca8d6]" : "bg-foreground/20"
                }`} />
                <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  {capability.status}
                </span>
              </div>
              <span className="font-medium block mb-1">{capability.name}</span>
              <span className="text-sm text-muted-foreground">{capability.detail}</span>
            </div>
          ))}
        </div>
      </div>
